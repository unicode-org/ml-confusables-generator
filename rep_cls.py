r"""Representation clustering module. Script for finding confusables."""
...
class RepCls:
    ...
    def __init__(self, embedding_file, label_file, img_dir, n_candidates=50,
        ...
        Args:
            ...
            secondary_filter_threshold: Float, only confusables with distance
                UNDER this threshold will be selected
        ...
        if len([name for name in self.__img_names if
    ...
    def label_file(self, label_file):
        """Check that label file exists and read into self.labels."""
    ...
    def img_dir(self, img_dir):
        ...
        self.__img_names = [os.path.join(img_dir, name) for name in
                            os.listdir(img_dir)]
        self.__img_dir = img_dir

        # Create map form label to image name
        __codepoints = [name.split('/')[-1].split('_')[0] for name in
                        self.__img_names]
        __keys = [chr(int('0x' + codepoint[2:], 16)) for codepoint in
                  __codepoints]
    ...
    def get_confusables_for_char(self, char):
        """Obtain confusables clustered around a certain character.
        ...
        for candidate in candidate_pool:
            if ord(char) == ord(candidate):
                continue
            if calculate_from_path(secondary_dis, self.__label_img_map[char],
                                   self.__label_img_map[candidate]) <= \
...
if __name__ == "__main__":
    ...
    rc = RepCls(embedding_file=args.embedding_file, label_file=args.label_file,
